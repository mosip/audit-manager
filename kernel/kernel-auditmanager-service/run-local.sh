#!/usr/bin/env bash
# kernel-auditmanager-service local runner — Linux, macOS, Windows Git Bash/MSYS.
...
PORT="${PORT:-8081}"
PID_FILE="${PID_DIR}/auditmanager-service.pid"
LOG_FILE="${LOG_DIR}/auditmanager-service.log"
MODULE="kernel-auditmanager-service"
PROFILE="${SPRING_PROFILES_ACTIVE:-local}"
CONFIG_URI="${SPRING_CLOUD_CONFIG_URI:-}"
CONFIG_LABEL="${SPRING_CLOUD_CONFIG_LABEL:-}"
LOADER_PATH="${loader_path_env:-.}"
IMAGE="${IMAGE:-kernel-auditmanager-service}"
CONTEXT="/v1/auditmanager"
...
usage() {
  cat <<EOF
Local kernel-auditmanager-service (${PROFILE} profile, :${PORT} ${CONTEXT})
...
print_endpoints() {
  echo
  echo "auditmanager  profile=${PROFILE}  port=${PORT}  context=${CONTEXT}"
...
cmd_start() {
  ...
  check_prereqs
  if is_running; then
    echo "auditmanager-service already running (pid $(cat "$PID_FILE")) on port ${PORT}"
    print_endpoints
    return 0
  fi
  rm -f "$LOG_FILE"
  local jar
  jar="$(find_boot_jar)"
  echo "==> starting auditmanager-service from $(basename "$jar")"
...
wait_ready() {
  ...
  while [[ "$elapsed" -lt "$timeout" ]]; do
    ...
    if grep -q "Started AuditManagerBootApplication" "$LOG_FILE" 2>/dev/null; then
...
cmd_stop() {
  ...
  local pid
  pid="$(cat "$PID_FILE")"
  echo "==> stopping auditmanager-service (${pid}) on port ${PORT}"
...
cmd_smoke() {
  ...
  local timeout="${1:-60}"
  local elapsed=0
  echo "==> waiting for auditmanager-service (up to ${timeout}s)"
  while [[ "$elapsed" -lt "$timeout" ]]; do
...
cmd_docker() {
  cmd_init
  need_cmd docker
  docker rm -f kernel-auditmanager-service >/dev/null 2>&1 || true
  echo "==> docker build ${IMAGE}"
  docker build -t "${IMAGE}" "${MODULE_DIR}"
  print_endpoints
  # shellcheck disable=SC2046
  exec docker run --rm -p "${PORT}:8081" --name kernel-auditmanager-service \
    $(docker_host_args) \
