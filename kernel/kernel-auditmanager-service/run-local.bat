...
REM Standalone Windows cmd runner for kernel-auditmanager-service (no .ps1).
...
set "PID_FILE=%PID_DIR%\auditmanager-service.pid"
set "LOG_FILE=%LOG_DIR%\auditmanager-service.log"
set "ERR_FILE=%LOG_DIR%\auditmanager-service.err.log"
set "MODULE=kernel-auditmanager-service"

if not defined PORT set "PORT=8081"
if not defined SPRING_PROFILES_ACTIVE set "SPRING_PROFILES_ACTIVE=local"
if not defined IMAGE set "IMAGE=kernel-auditmanager-service"
if not defined loader_path_env set "loader_path_env=."
set "CONTEXT=/v1/auditmanager"
...
:usage
echo Local kernel-auditmanager-service ^(%SPRING_PROFILES_ACTIVE% profile, :%PORT% %CONTEXT%^)
echo.
...
:print_endpoints
echo.
echo auditmanager  profile=%SPRING_PROFILES_ACTIVE%  port=%PORT%  context=%CONTEXT%
...
if not errorlevel 1 (
  ...
  echo auditmanager-service already running ^(pid !OLD_PID!^) on port %PORT%
  call :print_endpoints
  exit /b 0
)
call :port_in_use
if not errorlevel 1 (
  call :pid_on_port
  echo auditmanager-service already listening on port %PORT% ^(pid !PORT_PID!^)
...
echo ==^> starting auditmanager-service from %BOOT_JAR%
...
set "LAUNCH_PS1=%LOCAL_DIR%\launch-auditmanager-service.ps1"
> "%LAUNCH_PS1%" echo $ErrorActionPreference = 'Stop'
>> "%LAUNCH_PS1%" echo $argList = New-Object System.Collections.Generic.List[string]
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:-UseG1GC'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:-UseParallelGC'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:-UseShenandoahGC'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:+ExplicitGCInvokesConcurrent'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:+UseZGC'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:+ZGenerational'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:+UnlockExperimentalVMOptions'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:+UseStringDeduplication'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:+HeapDumpOnOutOfMemoryError'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:+UseCompressedOops'^)
>> "%LAUNCH_PS1%" echo $argList.Add^('-XX:MaxGCPauseMillis=200'^)
...
echo.
echo auditmanager-service ready
echo   port    %PORT%
call :print_endpoints
exit /b 0
...
if not errorlevel 1 (
  if exist "%LOG_FILE%" (
    findstr /C:"Started AuditManagerBootApplication" "%LOG_FILE%" >nul 2>&1
...
if not defined STOP_PID (
  echo auditmanager-service is not running
  exit /b 0
)
echo ==^> stopping auditmanager-service ^(%STOP_PID%^) on port %PORT%
...
:smoke
call :ensure_dirs
echo ==^> waiting for auditmanager-service ^(up to 60s^)
set /a ELAPSED=0
:smoke_loop
...
docker rm -f kernel-auditmanager-service >nul 2>&1
echo ==^> docker build %IMAGE%
docker build -t %IMAGE% "%MODULE_DIR%"
if errorlevel 1 exit /b 1
call :print_endpoints
docker run --rm -p %PORT%:8081 --name kernel-auditmanager-service -e active_profile_env=%SPRING_PROFILES_ACTIVE% -e spring_config_url_env=%SPRING_CLOUD_CONFIG_URI% -e spring_config_label_env=%SPRING_CLOUD_CONFIG_LABEL% -e JDK_JAVA_OPTIONS=%JDK_JAVA_OPTIONS% %IMAGE%
exit /b %ERRORLEVEL%
